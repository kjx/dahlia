...
twostate predicate allUnchanged(except : set<Object> := {})
 reads vs, ks, o, oHeap
 {
  &&  unchanged(vs - except)
  &&  unchanged(ks - except) 
...
//these dont work...
assert (forall xo <- a.extra, oo <- R[xo].AMFO :: R[oo] in (rowner.AMFO + rextra));
assert (forall xo <- a.extra, oo <- R[xo].AMFO :: (R[oo] in rowner.AMFO ) || (R[oo] in rextra));
